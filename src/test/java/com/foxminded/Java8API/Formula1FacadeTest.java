...
public class Formula1FacadeTest {
    ...
    private static final String RESULT = "Sebastian Vettel   | FERRARI                     | 01:04.415";
    ...
    private final Formula1Facade formula1Facade =
            new Formula1Facade(fileReader, parser, formatter,
                    ABBREVIATIONS_TXT_FILE_ADDRESS, START_RACE_LOG_FILE_ADDRESS, END_RACE_LOG_FILE_ADDRESS);
    ...
    void makeFormula1FacadeShouldCheckOrderAndMethodCalls() {
        ...
        when(fileReader.readTopRacers(START_RACE_LOG_FILE_ADDRESS)).thenReturn(FROM_START_LOG_FILE);
        when(fileReader.readTopRacers(END_RACE_LOG_FILE_ADDRESS)).thenReturn(FROM_END_LOG_FILE);
        when(fileReader.readTopRacers(ABBREVIATIONS_TXT_FILE_ADDRESS)).thenReturn(FROM_ABBREVIATION_TXT_FILE);

        radeDataDto = new RadeDataDto(fileReader.readTopRacers(START_RACE_LOG_FILE_ADDRESS),
